...
uint64_t calc_registry_sum(uint16_t * registry, uint64_t size)
{
    uint64_t sum = 0;
    ...
    Borad_t *boards, int64_t boardamount)
{
    ...
    uint16_t boardregistry[100] = {0};
    for (uint16_t idx = 0; idx < ARRAY_SIZE(boardregistry); idx++)
    {
    	boardregistry[idx] = 1;
    }

    printf("%ld\n",calc_registry_sum(boardregistry, ARRAY_SIZE(boardregistry)));
    ...
    for (int64_t jdx = 0; jdx < randseqsize; jdx++)
    {
        for (int64_t idx = 0; idx < boardamount; idx ++)
        {
            ...
            {
                boardregistry[idx] = 0;
                uint64_t boardsleft = calc_registry_sum(boardregistry, ARRAY_SIZE(boardregistry));
...
#       include "task_day4.inc"
    ...
    TEST_ASSERT_EQUAL(49686, sonar_sweep_day4_1(randseq, ARRAY_SIZE(randseq), boards, ARRAY_SIZE(boards)));
}
